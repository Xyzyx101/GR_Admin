describe('Cleaner', function() {
  ...
  describe('.Clean() ', function() {
    ...
    it('Should return date/time unchanged', function() {
      var now = new Date();
      expect(Cleaner.Clean(now)).toEqual(now);
    });
  ...
  describe('.ConvertDatesToTimestamps() ', function() {
    it(' should return boolean unchanged', function() {
      expect(Cleaner.ConvertDatesToTimestamps(true)).toBe(true);
      expect(Cleaner.ConvertDatesToTimestamps(false)).toBe(false);
    });

    it('Should return numbers unchanged', function() {
      expect(Cleaner.ConvertDatesToTimestamps(42)).toBe(42);
      expect(Cleaner.ConvertDatesToTimestamps(357.1234)).toBe(357.1234);
      expect(Cleaner.ConvertDatesToTimestamps(-1000.0)).toBe(-1000.0);
    });

    it('Should return null unchanged', function() {
      expect(Cleaner.ConvertDatesToTimestamps(null)).toBe(null);
    });

    it('Should convert date into timestamp', function() {
      var now = new Date();
      var stamp = now.getTime();
      expect(Cleaner.ConvertDatesToTimestamps(now)).toBe(stamp);
    });

    it('Should work with arrays', function() {
      var now = new Date();
      var stamp = now.getTime();
      var testMe = [now,now,now];
      var stamps = [stamp, stamp, stamp];
      expect(Cleaner.ConvertDatesToTimestamps(testMe)).toEqual(stamps);
    });

    it('Should work with objects', function() {
      var now = new Date();
      var stamp = now.getTime();
      var testMe = {'one':now,'two':now,'three':now};
      var stamps = {'one':stamp,'two':stamp,'three':stamp};
      expect(Cleaner.ConvertDatesToTimestamps(testMe)).toEqual(stamps);
    });

    it('Should find and convert deeply nested objects', function() {
      var now = new Date();
      var stamp = now.getTime();
      var veryDirty = {
        'obj1': {'prop':now},
        'obj2': {'otherProp': {'prop':now}},
        'obj3': {'otherOtherProp':  {'otherProp': {'prop':now}}}
      };
      var clean = {
        'obj1': {'prop':stamp},
        'obj2': {'otherProp': {'prop':stamp}},
        'obj3': {'otherOtherProp':  {'otherProp': {'prop':stamp}},}
      };
      expect(Cleaner.ConvertDatesToTimestamps(veryDirty)).toEqual(clean);
    });

    it('Should find and convert arrays of arrays', function() {
      var now = new Date();
      var nows = {'prop':now};
      var nowyNows = [nows, nows, nows]
      var nowyNowyNows = [nowyNows, nowyNows, nowyNows];

      var stamp = now.getTime();
      var stamps = {'prop':stamp};
      var stampyStamps = [stamps, stamps, stamps];
      var stampyStampyStamps = [stampyStamps, stampyStamps, stampyStamps];

      expect(Cleaner.ConvertDatesToTimestamps(nowyNowyNows)).toEqual(stampyStampyStamps);
    });
  });
});
